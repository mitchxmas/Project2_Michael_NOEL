...
const DailyWeatherPerDay = (props) => {
  ...
  return (
    ...
    >
      <div>{daysForecast[props.index]}</div>
      <div>{dateDDMM}</div>
      <div>{props.weather.daily.temperature_2m_max[props.index]}°C</div>
      <div>
        {" "}
        <img
          className={`icons ${styles.sunrise_sunset}`}
          src={iconSunrise}
        ></img>
        {sunriseTime}
      </div>
      ...
      <div>UV: {props.weather.daily.uv_index_max[props.index]}</div>
      <div>
        Rainfall: {props.weather.daily.precipitation_sum[props.index]}mm
      </div>
